...
```bash
# Run full unit test suite (56 tests, executes in < 1.5s):
...
| :--- | :--- | :---: |
| [`tests/test_api_routes.py`](tests/test_api_routes.py) | Health, BYOK, security headers (HSTS, CSP), MCP auth & callback provenance, ADK Director forward-call & fallback, quota preservation | **24/24 PASS** |
...
| **TOTAL** | **Comprehensive Build Integrity** | **56/56 PASS** |
